...
function one(svg,i){const c=svg.cloneNode(true);c.style.display="block";document.body.appendChild(c);let e="";c.querySelectorAll("path,polyline,polygon,rect,circle,ellipse").forEach(x=>{if(x.id==="sheet-bin"||x.closest("defs,clipPath,mask"))return;const p=pts(x);if(p.length>=2)e+=pl(p,"PART_"+(x.getAttribute("data-sheetnest-source-instance-id")||x.getAttribute("data-sheetnest-unit-id")||"DETAIL"))});c.remove();return"0\nSECTION\n2\nHEADER\n9\n$ACADVER\n1\nAC1009\n0\nENDSEC\n0\nSECTION\n2\nENTITIES\n999\nSheetNest layout "+(i+1)+"\n"+e+"0\nENDSEC\n0\nEOF\n"}
